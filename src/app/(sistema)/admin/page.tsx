...
import { Search, Shield, AlertCircle, Loader2, UserPlus, User, Power, KeyRound } from "lucide-react";
                            ...
                            {usuariosFiltrados.map((user) => (
                                <div key={user.id} className="p-5 bg-white hover:bg-gray-50 transition-colors">
                                    ...
                                    <div className="flex items-center mt-4 relative">
                                        <Shield className="w-3.5 h-3.5 text-slate-400 absolute left-3 pointer-events-none" />
                                        <select
                                            value={user.rol}
                                            onChange={(e) => handleRoleChange(user.id, e.target.value)}
                                            disabled={isRoleUpdating === user.id}
                                            className="appearance-none bg-slate-100 text-slate-700 border border-slate-200 rounded-full pl-8 pr-4 py-1 text-xs font-bold uppercase tracking-tighter outline-none focus:ring-2 focus:ring-red-600 cursor-pointer hover:bg-slate-200 transition-colors disabled:opacity-50"
                                        >
                                            ...
                                            <td className="p-5">
                                                <div className="flex items-center space-x-2">
                                                    <div className="relative inline-block">
                                                        <Shield className="w-3.5 h-3.5 text-slate-400 absolute left-2.5 top-1/2 transform -translate-y-1/2 pointer-events-none" />
                                                        <select
                                                            value={user.rol}
                                                            onChange={(e) => handleRoleChange(user.id, e.target.value)}
                                                            disabled={isRoleUpdating === user.id}
