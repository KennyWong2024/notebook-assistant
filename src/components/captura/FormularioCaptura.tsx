...
export default function FormularioCaptura() {
    ...
    useEffect(() => {
        const fetchFerias = async () => {
            ...
            try {
                ...
                if (!user) return;
                const { data } = await supabase.schema('sourcing').from('asignaciones_feria').select('id_feria, ferias!inner(*)').eq('id_usuario', user.id).eq('ferias.estado_activo', true);
                if (data && data.length > 0) {
                    const mapeadas = data.map(d => Array.isArray(d.ferias) ? d.ferias[0] : d.ferias) as unknown as Fair[];
                    setFeriasAsignadas(mapeadas);
                    setFeriaActiva(mapeadas[0]);
                }
            } catch (error) {
                console.error(error);
            } finally {
    ...
    // FUNCION RESTAURADA: Manejo del cierre seguro
    const handleCerrar = () => {
        ...
        try {
            ...
            for (const prod of productos) {
                ...
                const { data: dbProd } = await supabase.schema('sourcing').from('productos_prospecto').insert({
                    ...
                    creado_por: user.id,
                    nombre_rapido: prod.nombre.trim() || null, // null en lugar de "Sin nombre"
                    precio_referencia: prod.precio ? parseFloat(prod.precio) : null, // null en lugar de 0
                    moneda: prod.moneda,
        ...
        <div className="fixed inset-0 z-[100] bg-gray-50 overflow-y-auto animate-in slide-in-from-bottom-4 duration-300">
            ...
            <form onSubmit={handleSave} className="p-4 space-y-6 pb-32 max-w-md mx-auto">
                ...
                <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/80 backdrop-blur-lg border-t border-gray-100 z-[110]">
                    <button type="submit" disabled={loading || !nombreProveedor.trim()} className={`w-full max-w-md mx-auto py-5 rounded-[2rem] font-black text-xl shadow-xl transition-all flex items-center justify-center space-x-3 ${success ? "bg-green-500 text-white" : !nombreProveedor.trim() ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-red-600 text-white active:scale-95 shadow-red-200"}`}>
